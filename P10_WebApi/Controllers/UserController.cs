...
namespace P10_WebApi.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<IActionResult> UnFollow(string Userid)
        {
            ...
            var filterTargetUser = Builders<User>.Filter.Eq(u => u.UserId, Userid);

            var targetUser = await db.Users.Find(filterTargetUser).FirstOrDefaultAsync();
            ...
            var loggedUserObjId = ObjectId.Parse(loggedInUserId);
            var targetUserObjId = ObjectId.Parse(Userid);

            var alreadyFollowing = targetUser.Followers.Contains(loggedUserObjId);
            if (!alreadyFollowing)
            {
                return BadRequest(new { message = "You are not following this user!" });
            }
            var updateTarget = Builders<User>.Update.Pull(u => u.Followers, loggedUserObjId);

            var filterLoggedUser = Builders<User>.Filter.Eq(u => u.UserId, loggedInUserId);
            var updateLoggedUser = Builders<User>.Update.Pull(u => u.Following, targetUserObjId);

            await db.Users.UpdateOneAsync(filterTargetUser, updateTarget);
            await db.Users.UpdateOneAsync(filterLoggedUser, updateLoggedUser);
